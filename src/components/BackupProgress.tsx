...
/** Modal progress bar shown while a database is being backed up to a `.dbbak`
 * archive (driven by the store's `backupProgress` state). */
export function BackupProgress() {
  const backupProgress = useStore((s) => s.backupProgress);
  const cancelBackup = useStore((s) => s.cancelBackup);
  if (!backupProgress) return null;

  const { database, table, tableIndex, tableCount, done, total, cancelling } =
    backupProgress;
      ...
      >
        <div className="flex items-center gap-2 text-[13px] text-zinc-100">
          ...
          <span className="truncate">
            Backing up <span className="font-semibold">{database}</span>
          </span>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          <span className="text-[11px] tabular-nums text-zinc-500 truncate">
            {cancelling
              ? "Cancelling…"
              : tableCount > 0
              ? `${table} (${tableIndex}/${tableCount})${
                  total > 0
