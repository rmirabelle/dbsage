...
export function FolderDeleteDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      notifyError(`Could not delete folder: ${String(e)}`);
      setBusy(false);
    }
  };
  ...
  return (
    ...
    >
      <div
        data-el="folder-delete-dialog"
        role="dialog"
        aria-modal="true"
        onClick={(e) => e.stopPropagation()}
        className="w-[440px] max-w-[90vw] rounded-lg border border-rose-900/60 bg-zinc-900 shadow-2xl shadow-black/60"
      >
        ...
        <div className="px-4 py-4 text-[12px] leading-relaxed text-zinc-300">
          {tableCount > 0 ? (
            <p>
              ...
              <span className="font-semibold text-zinc-100">{tableCount}</span>{" "}
              table{tableCount === 1 ? "" : "s"}. Deleting the folder will{" "}
              <span className="font-semibold text-zinc-100">not</span> delete those
              tables — they’ll move back to the root table list.
            </p>
          ) : (
            <p>This empty folder will be removed. No tables are affected.</p>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-zinc-800 px-4 py-3">
          ...
          <button
            data-el="folder-delete-confirm-btn"
