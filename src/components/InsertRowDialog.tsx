...
export function InsertRowDialog({
  ...
  useEffect(() => {
    ...
    ipc
      ...
      .then((cols) => {
        ...
        for (const c of cols) init[c.name] = "";
        setFields(init);
      })
      .catch((e) => {
  ...
  const submit = async () => {
    ...
    for (const m of meta) {
      ...
      /* No value supplied: auto-increment fills itself; a nullable column gets
         NULL; a NOT NULL column with a default uses it; otherwise it's required. */
          ...
          {loadError ? (
            ...
            <div className="space-y-3">
              {meta.map((m) => {
                ...
                return (
                  <div key={m.def.name} className="space-y-1">
                    ...
                    <input
                      value={fields[m.def.name] ?? ""}
                      disabled={busy}
                      onChange={(e) => setField(m.def.name, e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          submit();
                      ...
                      className="w-full bg-zinc-950 border border-zinc-700 rounded px-2 py-1 font-mono text-zinc-100 outline-none focus:border-accent-500 disabled:opacity-50"
                    />
                  </div>
                );
              })}
