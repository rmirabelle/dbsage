...
export const JsonTreeView = memo(function JsonTreeView({ data, search, activeIndex }: Props) {
  const rows = useMemo(() => buildJsonTreeRows(data), [data]);
  const [collapsed, setCollapsed] = useState(() => new Set(rows.filter((row) => row.container && !(Array.isArray(data) && row.path === "/0")).map((row) => row.path)));
        ...
        {virtualizer.getVirtualItems().map((item) => {
          ...
          return <div key={item.key} data-index={item.index} ref={virtualizer.measureElement}
            ...
            <div className={clsx("flex items-start gap-1", row.container && "rounded cursor-pointer hover:bg-zinc-900/50")}
              ...
              {row.path !== "" && <span className={clsx("shrink-0", row.arrayIndex ? "text-zinc-600" : "text-sky-300")}>{row.arrayIndex ? row.key : highlight(row.key, match.keys, search.length, match.start, activeIndex)}<span className="text-zinc-600">:</span></span>}
              {row.container ? <span className="text-zinc-600">{Array.isArray(row.value) ? `[${row.count}]` : `{${row.count}}`}</span>
