...
export function ZoomControls({ pane }: { pane: PaneId }) {
  ...
  return (
    <div className="flex items-center gap-px text-zinc-500">
      <button
      ...
      >
        {isDefault ? "100%" : <span className="inline-flex items-center gap-1"><RotateCcw size={9} />{Math.round(zoom * 100)}%</span>}
      </button>
      <button
        onClick={() => bumpZoom(pane, ZOOM_BOUNDS.STEP)}
        disabled={atMax}
